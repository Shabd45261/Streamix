...
@Singleton
class MovieboxProvider @Inject constructor() : BaseScraper() {

    override val name = "Moviebox"
    override val mainUrl = "https://moviebox.ng"
    ...
    override suspend fun search(query: String): List<SearchResult> {
        return try {
            val q = if (query.isEmpty()) "trending" else query
            val encoded = URLEncoder.encode(q, "UTF-8")
            val doc = fetchDoc("$mainUrl/search?q=$encoded", mainUrl)
            parseMovieCards(doc)
        } catch (e: Exception) {
            emptyList()
        }
    ...
    private fun parseMovieCards(doc: Document): List<SearchResult> {
        return doc.select("li.videoblock, div.movie-card, article, .film-card").mapNotNull { el ->
            ...
            val title = el.selectFirst("h3, .title, .movie-title")?.text()?.trim() ?: linkEl.attr("title") ?: ""
            ...
            SearchResult(
                ...
                year = el.selectFirst(".year, .movie-year")?.text() ?: ""
            )
        }.distinctBy { it.id }
    }

...
class SearchViewModel @Inject constructor(
    ...
    fun search() {
        ...
        viewModelScope.launch {
            ...
            try {
                val tmdbResults = async { tmdbRepo.search(q) }
                val okxxxResults = async { okxxx.search(q) }
                val pornhatResults = async { pornhat.search(q) }
                val youtubeResults = async { youtube.search(q) }
                
                val combined = awaitAll(tmdbResults, okxxxResults, pornhatResults, youtubeResults)
                    .flatten()
                    .distinctBy { it.id }
